...
from doctrail.llm_operations import process_row, process_row_structured
...
async def test_prompt_template_substitution():
    ...
    with patch('doctrail.llm_operations.call_llm', side_effect=mock_call_llm):
...
async def test_prompt_template_with_table_prefix():
    ...
    with patch('doctrail.llm_operations.call_llm', side_effect=mock_call_llm), \
         patch('doctrail.llm_operations.apply_column_limits', side_effect=mock_apply_column_limits):
...
@pytest.mark.asyncio
async def test_prompt_template_uses_runtime_and_config_variables():
    """Prompt templates should support model, table, and scalar config variables."""
    row = {
        'rowid': 1,
        'sha1': 'test123',
        'raw_content': 'A short document.'
    }
    ...
    with patch('doctrail.llm_operations.call_llm', side_effect=mock_call_llm):
        await process_row(
            row=row,
            input_cols=['raw_content'],
            parsed_input_cols=[('raw_content', None)],
            prompt='Use {model} on {table} for {project_name}: {raw_content}',
            model='gpt-4o-mini',
            semaphore=semaphore,
            pbar=pbar,
            output_col='result',
    ...
    user_message = captured_messages[0]['content']
    assert "Use gpt-4o-mini on documents for demo project: A short document." in user_message
    assert "{model}" not in user_message
    assert "{table}" not in user_message
    assert "{project_name}" not in user_message
