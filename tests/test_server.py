...
class TestServerContracts:
    ...
    def test_web_ingestor_quotes_table_names(self, tmp_path, monkeypatch, caplog):
        """The web ingestor summary queries should handle valid quoted SQLite table names."""
        from doctrail.server_ingestor.app import create_app

        db_path = tmp_path / "upload.db"

        async def fake_process_ingest(**kwargs):
            conn = sqlite3.connect(kwargs["db_path"])
            conn.execute(
                """
                CREATE TABLE IF NOT EXISTS "my-docs" (
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO "my-docs"
                (sha1, filename, filepath, raw_content, file_created, file_modified)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                ("abc", "doc.txt", "doc.txt", "hello world", "now", "now"),
            )
            conn.commit()
            conn.close()
            return {"status": "success"}

        monkeypatch.setattr("doctrail.server_ingestor.app.process_ingest", fake_process_ingest)
        app = create_app()

        with TestClient(app) as local_client:
            response = local_client.post(
                "/ingest",
                data={"db_path": str(db_path), "table_name": "my-docs"},
                files=[("files", ("doc.txt", b"hello world", "text/plain"))],
            )

        assert response.status_code == 200
        assert response.json()["table_name"] == "my-docs"
        assert not any("Could not query database" in record.message for record in caplog.records)
