...
def should_skip_file(file_path: str) -> bool:
    ...
    # Check for actual sync conflict files (more specific pattern)
    # These typically have patterns like "conflicted copy" or "(conflict)"
    if 'conflicted copy' in path.name.lower() or '(conflict)' in path.name.lower():
        logger.info(f"Skipping sync conflict file: {file_path}")
        return True
...
def apply_file_patterns(files: List[Path], include_pattern: Optional[str] = None, exclude_pattern: Optional[str] = None) -> List[Path]:
    ...
    if include_pattern:
        ...
        logger.info(
            "Include pattern(s) '%s' matched %d files",
            include_pattern,
            len(filtered_files),
        )
...
def get_unsupported_file_error(file_path: str) -> str:
    ...
    error_messages = {
        ...
        '.csv': 'CSV extraction failed. The file may be corrupted or encoded in an unsupported format.',
        '.tsv': 'TSV extraction failed. The file may be corrupted or encoded in an unsupported format.',
        # '.rtf' is now supported via textutil/unrtf
