...
def find_doctrail_env_file(start_dir: Optional[str | Path] = None) -> Optional[Path]:
...
def _apply_env_file(env_path: Path, *, override: bool) -> None:
    ...
    Mutates os.environ for this process (and any children) only — it never touches the
    user's shell, so secrets stay scoped to doctrail invocations.
    """
...
def resolve_enrichment_prompt(enrichment_config: Dict[str, Any], config_data: Dict[str, Any]) -> str:
    """Return the effective prompt text, including append_file content when configured."""
    prompt_text = enrichment_config.get('prompt', '')
    append_file_path = enrichment_config.get('append_file')
    if not append_file_path:
        return prompt_text

    if not os.path.isabs(append_file_path) and config_data.get('__config_path__'):
        config_dir = os.path.dirname(config_data['__config_path__'])
        append_file_path = os.path.join(config_dir, append_file_path)

    with open(append_file_path, 'r', encoding='utf-8') as handle:
        appended_content = handle.read()
    return prompt_text + "\n\n" + appended_content
