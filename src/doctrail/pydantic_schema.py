...
from typing import Dict, Any, Type, List, Optional, Union, get_args, get_origin
from pydantic import BaseModel, ConfigDict, Field, create_model, field_validator, ValidationError
...
def yaml_to_pydantic_type(
    yaml_type_def: Union[str, Dict[str, Any]],
    ...
    if isinstance(yaml_type_def, dict):
        ...
        if yaml_type_def.get("optional", False) or yaml_type_def.get("nullable", False):
    ...
    if base_type in ["array", "list"]:
        ...
        if items_def:
            if isinstance(items_def, str):
                ...
                # Check for structured object items (type: "object" with properties)
                if items_def.get("type") in ("object", "dict") and "properties" in items_def:
                    # Create a nested Pydantic model for the array item type.
                    # This produces proper JSON Schema with additionalProperties: false,
                    # which is required by OpenAI's structured output API.
                    nested_name = f"NestedItem_{abs(hash(str(items_def))) % 100000}"
                    item_type = create_pydantic_model_from_schema(
                        items_def["properties"],
                        nested_name,
                        all_fields_optional=all_fields_optional,
                    )
                else:
                    item_type, _, item_lang, item_convert = yaml_to_pydantic_type(
                        items_def,
                        all_fields_optional=all_fields_optional,
                    )
...
def create_pydantic_model_from_schema(schema: Dict[str, Any], model_name: str = "DynamicModel", all_fields_optional: bool = False) -> Type[BaseModel]:
    ...
    for field_name, field_def in schema.items():
        try:
            python_type, field_kwargs, lang, convert = yaml_to_pydantic_type(
                field_def,
                all_fields_optional=all_fields_optional,
            )
    ...
    try:
        # Create the dynamic model with extra='forbid' so that Pydantic emits
        # additionalProperties: false in JSON Schema (required by OpenAI structured output)
...
def analyze_schema_complexity(schema: Dict[str, Any], output_column: str = None) -> Dict[str, Any]:
    """
    Analyze schema to determine storage strategy.

    Args:
        schema: Schema dictionary (either JSON Schema-style or field definitions)
        output_column: Optional output column name (used for bare type schemas)
    ...
    Returns:
        {
            ...
            "is_bare_type": bool  # True if schema is {type: "string"} style
        }
    """
    if not isinstance(schema, dict):
        return {
    ...
    # Detect JSON Schema-style bare type declarations like {type: "string"}
    # or bare enum schemas like {enum: ["a", "b"]}
    # vs field definitions like {summary: {type: "string"}}
    PRIMITIVE_TYPES = {"string", "integer", "number", "boolean", "array", "object"}
    is_bare_type = (
        # {type: "string"} style
        ("type" in schema and isinstance(schema.get("type"), str) and schema.get("type") in PRIMITIVE_TYPES) or
        # {enum: [...]} style
        ("enum" in schema and isinstance(schema.get("enum"), list)) or
        # {enum_list: [...]} style
        ("enum_list" in schema and isinstance(schema.get("enum_list"), list))
    )

    if is_bare_type:
        # This is a bare type schema like {type: "string"} or {type: "string", maxLength: 500}
        # Use output_column as the field name, or "value" as fallback
        field_name = output_column or "value"
        return {
            "field_count": 1,
            "is_simple": True,
            "is_complex": False,
...
def get_sql_type_from_pydantic_type(python_type: Type) -> str:
    ...
    origin = get_origin(python_type) or getattr(python_type, '__origin__', None)

    if origin is Union:
        non_none_args = [arg for arg in get_args(python_type) if arg is not type(None)]
        if len(non_none_args) == 1:
            return get_sql_type_from_pydantic_type(non_none_args[0])
        return "TEXT"
