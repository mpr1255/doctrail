...
class Plugin:
    ...
    async def ingest(
        ...
        **kwargs
        ...
        if not overwrite:
            ...
            # - New items (not in DB)
            # - Items without full text (can be updated with PDFs)
            # - Items without abstracts (can be updated from OpenAlex)

            try:
                # Get existing items with their content status (more efficiently)
                existing_items = {}
                query = """
                    SELECT zotero_key,
                ...
                for row in db.execute(query).fetchall():
                    key = row[0]
                    has_fulltext = row[1] and row[1] > 100
                    has_abstract = row[2] and row[2] > 10
                    existing_items[key] = {
                ...
                for item in items:
                    ...
                    elif not existing_items[key]['has_fulltext']:
                        # Existing but no full text - process to potentially add PDF
                        items_to_process.append(item)
                        logger.debug(f"Will update {key}: missing full text")
                    elif not existing_items[key]['has_abstract']:
                        # Existing but no abstract - process to potentially fetch from OpenAlex
                        items_to_process.append(item)
                        logger.debug(f"Will update {key}: missing abstract")
                    else:
                        # Item is complete
                        items_complete += 1

                skipped_existing = items_complete
                logger.info(f"Items to process: {len(items_to_process)} (new or needing updates)")
                logger.info(f"Items already complete: {items_complete}")

            except Exception as e:
                logger.warning(f"Could not check existing items: {e}")
                items_to_process = items  # Continue with all items if we can't check
        else:
            # Overwrite mode - process all items
        ...
        console.print(Panel.fit(
            ...
            f"To process: [green]{len(items_to_process)}[/green] (new or needing updates)\n"
            f"Already complete: [yellow]{skipped_existing}[/yellow]\n"
        ...
        # Process statistics
        success_with_fulltext = 0
        success_with_abstract = 0
        success_metadata_only = 0
        error_count = 0
        skipped_count = 0
        failed_items = []
        ...
        with tqdm(total=len(items_with_metadata), desc="Processing items locally") as pbar:
            for item_data in items_with_metadata:
                item = item_data['item']
                item_title = item['data'].get('title', 'Unknown')
                pbar.set_description(f"Processing: {item_title[:50]}")
                ...
                try:
                    ...
                    if result['status'] == 'success_with_fulltext':
                        success_with_fulltext += 1
                    elif result['status'] == 'success_with_abstract':
                        success_with_abstract += 1
                    elif result['status'] == 'success_metadata_only':
                        success_metadata_only += 1
                    elif result['status'] == 'skipped':
                        skipped_count += 1
                    else:
                        # Unexpected status
                        error_count += 1
                        failed_items.append((item['key'], item_title, result.get('status', 'unknown')))

                except Exception as e:
                    logger.error(f"Error processing item {item['key']}: {e}")
                    error_count += 1
                    failed_items.append((item['key'], item_title, str(e)))

                pbar.update(1)
        
        # Final summary
        total_skipped = skipped_count + skipped_existing
        total_success = success_with_fulltext + success_with_abstract + success_metadata_only

        console.print(Panel.fit(
            f"[bold]Ingestion Complete[/bold]\n\n"
            f"Total items in collection: [cyan]{len(items)}[/cyan]\n"
            f"Already in database: [yellow]{skipped_existing}[/yellow]\n"
            f"Items processed: [cyan]{len(items_with_metadata)}[/cyan]\n\n"
            f"[bold]Successfully Ingested:[/bold] [green]{total_success}[/green]\n"
            f"  • With full text: [green]{success_with_fulltext}[/green]\n"
            f"  • With abstract only: [yellow]{success_with_abstract}[/yellow]\n"
            f"  • Metadata only: [dim]{success_metadata_only}[/dim]\n\n"
            f"Skipped (already exists): [yellow]{total_skipped}[/yellow]\n"
            f"Errors: [red]{error_count}[/red]",
            title="Complete",
            border_style="green"
        ))

        # If there were errors, show which items failed
        if failed_items:
            console.print("\n[bold red]Failed Items:[/bold red]")
            for key, title, error in failed_items:
                console.print(f"  • {key}: {title[:50]}... - {error}")

        return {
            "total": len(items),
            "success_with_fulltext": success_with_fulltext,
            "success_with_abstract": success_with_abstract,
    ...
    async def _fetch_openalex_abstract(self, doi: str) -> Optional[str]:
        ...
        try:
            ...
            if doi.startswith("https://doi.org/"):
                ...
                clean_doi = doi.replace("http://doi.org/", "")

            # Build OpenAlex API URL
            encoded_doi = urllib.parse.quote(f"https://doi.org/{clean_doi}", safe='')
            url = f"https://api.openalex.org/works/{encoded_doi}"

            # Add email for polite pool (optional but recommended)
            headers = {
                'User-Agent': 'Doctrail/1.0 (mailto:doctrail@example.com)'
            }
            ...
            with urllib.request.urlopen(req, timeout=10) as response:
                ...
                if 'abstract_inverted_index' in data and data['abstract_inverted_index']:
                    abstract_dict = data['abstract_inverted_index']
                ...
                # Fallback to abstract field if available
    ...
    async def _process_item_local(
        ...
        # Extract bibliographic metadata first
        ...
        # Fetch abstract from OpenAlex if not available locally
        if not abstract and doi:
            logger.debug(f"Fetching abstract from OpenAlex for DOI: {doi}")
            abstract = await self._fetch_openalex_abstract(doi)
            if abstract:
                logger.debug(f"Retrieved abstract from OpenAlex for {item_key} ({len(abstract)} chars)")

        # We always want to save items even without attachments or abstracts
        # They still have valuable metadata (title, authors, DOI, etc.)
                ...
                if target_file and target_file.exists():
                    ...
                    try:
                        ...
                        if extracted_text and len(extracted_text) > 100:
                            ...
                            # Extract the processing method from metadata
                            if metadata and isinstance(metadata, dict):
                                extraction_method = metadata.get('processing_method') or metadata.get('extraction_method')
                            break
                    except Exception as e:
                        logger.warning(f"Failed to extract text from {target_file}: {e}")
        
        # Always save metadata even without full text (abstract may come from OpenAlex)
        ...
        record = {
            ...
            "doi": doi,
            "abstract": abstract,  # May come from Zotero or OpenAlex
            ...
            "collection": data.get('collections', []),  # This is actually collection keys
            "item_type": data.get('itemType'),
            "tags": json.dumps([tag['tag'] for tag in data.get('tags', [])]),
            "ingested_at": datetime.now().isoformat()
        }
        
        # Smart update: only update fields that are NULL or empty in the database
        existing = list(db[self.target_table].rows_where("zotero_key = ?", [item_key]))
        ...
        if existing and not overwrite:
            ...
            for field, value in record.items():
                # Skip primary key and timestamp
                if field in ['sha1', 'zotero_key', 'ingested_at']:
                    continue

                # Update if field is NULL, empty string, or None in database
                existing_value = existing_record.get(field)
                if existing_value is None or existing_value == '' or existing_value == []:
                    if value is not None and value != '' and value != []:
                        update_fields[field] = value
                        logger.debug(f"Will update {field}: {existing_value} -> {value[:50] if isinstance(value, str) else value}")

            if update_fields:
                logger.debug(f"Updating {len(update_fields)} fields for {item_key}: {list(update_fields.keys())}")
                # Use the primary key 'sha1' for the update
                primary_key = existing_record['sha1']
                db[self.target_table].update(primary_key, update_fields, alter=True)
                logger.info(f"Updated {len(update_fields)} fields for {item_key}")
            else:
                logger.debug(f"No fields to update for {item_key}")
        else:
            # Insert new record or replace if overwrite=True
            db[self.target_table].insert(record, replace=True)
            logger.debug(f"Inserted/replaced record for {item_key}")
        
        # Determine status based on what we found
        status = 'success'
        if full_text:
            status = 'success_with_fulltext'
        elif abstract:
            status = 'success_with_abstract'
        else:
            status = 'success_metadata_only'

        return {'status': status, 'item_key': item_key, 'overwrite': overwrite}
    ...
    def _ensure_literature_schema(self, db_path: str) -> None:
        ...
        if self.target_table not in db.table_names():
            db[self.target_table].create({
                ...
                "zotero_metadata": str,
                "collection": str,  # Collection keys from Zotero
                "item_type": str,
                "tags": str,
            ...
            # Rename collection_name to collection if it exists
            if "collection_name" in columns and "collection" not in columns:
                db.execute(f"ALTER TABLE {self.target_table} RENAME COLUMN collection_name TO collection")
                logger.info(f"Renamed column collection_name to collection")

            # Add extraction_method column if missing
            if "extraction_method" not in columns:
                try:
                    table.add_column("extraction_method", str)
                    logger.info(f"Added extraction_method column")
                except Exception:
