...
from doctrail.ingest import clean_metadata as ingest_clean_metadata
...
async def process_zotero_ingest(
    ...
    console.print(Panel.fit(
        "[bold red]ZOTERO INGESTER DISABLED[/bold red]\n\n"
    ...
    try:
        ...
        if not overwrite:
            if zotero_table.exists() and "raw_content" in zotero_table.columns_dict:
                try:
                    # Fetch keys where content is NOT NULL and not empty
                    query = f"SELECT zotero_key FROM {db.quote(table_name)} WHERE raw_content IS NOT NULL AND raw_content != ''"
