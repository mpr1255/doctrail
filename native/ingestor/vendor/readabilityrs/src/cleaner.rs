...
///
/// Note: forms are intentionally NOT stripped here (see the body); Mozilla keeps
/// them and prunes junky ones later via cleanConditionally.
///
...
pub fn prep_document(html: &str) -> String {
    ...
    // NOTE: do not blanket-remove <form>…</form>. Mozilla Readability keeps forms
    // and prunes junky ones via cleanConditionally. ASP.NET WebForms sites (very
    // common on Chinese government portals) wrap the entire page body in a single
    // <form id="form1" runat="server">, so stripping forms here deletes the whole
    // article and leaves only the out-of-form nav/footer.
